...
<head>
  ...
  <script type="text/javascript">
 	var ctx = '${pageContext.request.contextPath }';
  	function steal(obj){
  		console.log(obj.id);
  		$.post(ctx + '/mobile/user/steal', {'steal':obj.id}, function(data){
			alert(data.message);
...
<body>
  <div class="app app-header-fixed" id="app">
    ...
    <div class="app-content">
    	...
    	<h3 class="text-center"><strong>我的上级</strong></h3>
    	<table class="table table-bordered table-striped">
		    <tr class="text-center active">
	   		...
	   		<tr class="text-center active">
		    	<td>${upUser.realName }</td>
		    	<td>${upUser.userPhone }</td>
		    	<td>${upUser.addTime }</td>
		    	<td>
		    	<input class="btn btn-primary" id='${upUser.userId }' type='button' onclick='steal(this)' value='偷取果实' />
		    	</td>
	   		</tr>
	    </table>
    	<h3 class="text-center"><strong>我的下级</strong></h3>
    	<table class="table table-bordered table-striped">
		    <tr class="text-center active">
		    	<td><strong>用户名</strong></td>
	            <td><strong>手机号</strong></td>
    		...
    		<c:forEach items="${downUser }" var="user">
	    		<tr class="text-center active">
			    	<td>${user.realName }</td>
			    	<td>${user.userPhone }</td>
			    	<td>${user.addTime }</td>
			    	<td>
			    		<input class="btn btn-primary" id='${user.userId }' type='button' onclick='steal(this)' value='偷取果实' />
			    	</td>
	    		</tr>
    		</c:forEach>
	    </table>
